...
def get_synthetic_train_transforms(seed: Union[int, None] = None,
    ...
    transform_list = [
        ...
        #Printerd(keys=["img"], message= "RandomLabelsToImage"),
        RandGaussianSmoothd(keys="img", sigma_x=(0.5, 1.5), prob=0.3),
        #Printerd(keys=["img"], message= "RandomGaussianSmooth"),
        ...
        #Printerd(keys=["img"], message= "Elastic"),
        ...
        # RandMotiond(keys=["img"], degrees=5, translation=0.001, num_transforms=1, perturbation=0.1,
        #     image_interpolation='linear', prob=1.0),#0.02),
        ...
        #Printerd(keys=["img"], message= "Cleanlabels"),
        FgBgToIndicesd(keys=['seg']),
        # # Crop random fixed sized regions with the center being a foreground or background voxel
        # # based on the Pos Neg Ratio.
        RandCropByPosNegLabeld(keys=['img', 'seg', 'instance_seg', 'labels', 'brainmask'],
                               ...
                               spatial_size=patch_size, num_samples=1,
                               pos=1, neg=1, allow_smaller=True),
        SpatialPadd(keys=['img', 'seg', 'instance_seg', 'labels', 'brainmask'], spatial_size=patch_size),
        LesionOffsetTransformd(keys="instance_seg"),
